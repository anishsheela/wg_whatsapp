...
export function startScheduler(sock) {
  ...
  cron.schedule('0 11 * * *',
    () => closeKitchenDay(sock, g).catch(console.error));

  // ── Full Clean ─────────────────────────────────────────────────────────────
  cron.schedule(`0 ${s.fullCleanHour} * * ${s.weeklyDay}`,
    () => assignFullClean(sock, g).catch(console.error));

  cron.schedule(`0 ${s.weeklyReminderHour} * * ${s.weeklyDay}`,
    () => remindFullClean(sock, g).catch(console.error));

  cron.schedule(`0 0 * * ${dayAfterWeekly}`,
    () => closeFullCleanWeek(sock, g).catch(console.error));

  // ── Toilet ─────────────────────────────────────────────────────────────────
  cron.schedule(`0 ${s.toiletHour} * * ${s.weeklyDay}`,
    () => assignToilet(sock, g).catch(console.error));

  cron.schedule(`5 ${s.weeklyReminderHour} * * ${s.weeklyDay}`,
    () => remindToilet(sock, g).catch(console.error));

  cron.schedule(`5 0 * * ${dayAfterWeekly}`,
    () => closeToiletWeek(sock, g).catch(console.error));
